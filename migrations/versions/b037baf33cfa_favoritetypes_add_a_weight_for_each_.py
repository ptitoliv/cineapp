...
def upgrade():
    ...
    conn = op.get_bind()
    res=conn.execute(sa.text("UPDATE favorite_types SET star_weight=1 WHERE star_type=\"favorite_star\""))
    res=conn.execute(sa.text("UPDATE favorite_types SET star_weight=2 WHERE star_type=\"homework_star\""))
    res=conn.execute(sa.text("UPDATE favorite_types SET star_weight=3 WHERE star_type=\"mustsee_star\""))
